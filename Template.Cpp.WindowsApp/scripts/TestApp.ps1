...
Set-Variable -Scope Script -Name 'SetAclPath' -Value $Path
...
Add-Type -AssemblyName System.Windows.Controls.Image

#$MsgBoxPath = $env:ScriptsRoot + '\Extended-MessageBox.ps1'
#. $MsgBoxPath
...
Function Get-MsgBox-Result 
{
    $Result = Get-Variable -Name PWSHMessageBoxOutput -ValueOnly -ErrorAction Stop
    return $Result
}
...
function Show-MessageBox-Error
{
<#
    .SYNOPSIS
...
function Show-MessageBox-Warning
{
<#
    .SYNOPSIS
    Display a mesage box with colors to highlight a ERROR message
...
#>
 ...
 $WarningParams = @{
    ...
    Show-MessageBox @WarningParams -Content $Text
}
...
$ImageWarning = "$CurrentPath\img\warning.png"
$BannerImagePath = "$CurrentPath\img\banner.jpg"

function Show-Success($DestinationPath)
{
    $InfoParams = @{
        Title = "DONE!"
        TitleFontSize = 20
    ...
    Show-MessageBox @InfoParams -Content "Project Generated at "
...
#>
    $me=whoami
    $SetAclExe = 'C:\MyApps\Shims\SetACL.exe'
    $ArgumentList = '-on "' + $Script:SetAclPath + '" -ot file -actn setowner -ownr "n:' + $me + '" -actn ace -ace "n:' + $me + ';p:full" -rec cont_obj'
     


    $var_TextPath.Text = $Script:SetAclPath
    ...
    $var_BtnGenerate.Add_Click( {
     $var_Log.Text = Start-Process -FilePath $SetAclExe -ArgumentList $ArgumentList -Wait -NoNewWindow| Out-String
       })


    $window.Title = "Set Folder Ownership"
